    ...
    divPai.innerHTML = `
        <div class="flex flex-row items-center gap-y-4 gap-x-6 mx-2 place-content-between">
            ...
            <div class="flex flex-row gap-x-2 items-center">
                ...
                <svg data-favoritar-termo xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="w-8 h-8 cursor-pointer p-1 rounded-lg ${setFavoritado} hover:bg-gray-200 stroke-2 stroke-laranja-fraco">
                    <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
                </svg>
                <img data-deletar-termo class="w-7 h-7 cursor-pointer p-1 rounded-lg hover:bg-gray-200" src="${iconeDeletar}" 
        ...
        <div class="mx-2">
            <p data-descricao class="text-preto-padrao text-justify">${descricao}</p>
        </div>
    `
    const btnFavoritar = divPai.querySelector('[data-favoritar-termo]');
    btnFavoritar.addEventListener('click', (e) => {
        favoritarCard(e.target);
    });

    const btnEditarTermo = divPai.querySelector('[data-editar-termo]');
    btnEditarTermo.addEventListener('click', (e) => {
        exibirModalEdicao(e.target);
    });

    const btnDeletar = divPai.querySelector('[data-deletar-termo]');
